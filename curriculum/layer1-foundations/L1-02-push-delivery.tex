...
When an iOS app calls \texttt{UIApplication.shared.registerForRemoteNotifications()}, the OS sends a request to APNs. APNs generates a \textbf{device token}: a unique, opaque byte string that identifies the combination of this specific device and this specific app. The OS delivers the token to your app via the \texttt{application(\_:\allowbreak{}didRegisterForRemoteNotificationsWithDeviceToken:)} delegate callback. Your app is responsible for forwarding this token to your backend server, which stores it.
